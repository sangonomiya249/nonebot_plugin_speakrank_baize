...
async def send_image_safe(bot: Bot, group_id: int, img_bytes: bytes,
                          wait_max: float = 3.0, wait_step: float = 0.15):
    """直接发送图片 bytes（内存直发，不写磁盘）"""
    if not img_bytes or len(img_bytes) < 100:
        logger.error(f"图片数据异常 size:{len(img_bytes)}")
    ...
    try:
        ...
        logger.info(f"图片发送成功 ({len(img_bytes) // 1024} KB)")
