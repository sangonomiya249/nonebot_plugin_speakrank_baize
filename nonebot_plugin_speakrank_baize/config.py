...
class Config:
    ...
    # ── 数据存储 ──
    data_path: Path = field(default_factory=lambda: Path(_env("DATA_PATH", str(_PLUGIN_DIR / "data" / "activity_stat"))))  # SQLite 数据库存放目录
    ...
    # ── 群聊发送模式 ──
    send_mode: str = field(default_factory=lambda: _env("MODE", 'whiltelist'))  # "whitelist"(仅白名单群) / "blacklist"(所有群，黑名单例外)

    # ── 图片输出 ──
    image_dir: Path = field(default_factory=lambda: Path(_env("IMAGE_DIR", str(_PLUGIN_DIR / "activity_images"))))  # 生成的排行图片和折线图存放目录
    ...
    # ── 排行限制 ──
    max_display_rank: int = field(default_factory=lambda: _env("MAX_DISPLAY_RANK", 8))  # 排行榜图片最多显示前 N 名
...
CONFIG = Config(
    data_path=Path(_env("DATA_PATH", str(_PLUGIN_DIR / "data" / "activity_stat"))),  # SQLite 数据库存放目录
    ...
    send_mode=_env("MODE", 'whiltelist'),  # 群聊发送模式: blacklist(默认所有群) / whitelist(仅白名单)
    image_dir=Path(_env("IMAGE_DIR", str(_PLUGIN_DIR / "activity_images"))),  # 生成的排行图片和折线图存放目录
    ...
    max_display_rank=_env("MAX_DISPLAY_RANK", 8),  # 排行榜图片最多显示前 N 名
...
def reload_config():
    ...
    for _env_file in [_root / ".env", _root / f".env.{os.environ.get('ENVIRONMENT', '')}"]:
        try:
            ...
            pass
    CONFIG = Config(
        data_path=Path(_env("DATA_PATH", str(_PLUGIN_DIR / "data" / "activity_stat"))),
        ...
        image_dir=Path(_env("IMAGE_DIR", str(_PLUGIN_DIR / "activity_images"))),
